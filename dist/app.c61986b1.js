...
function newsFeed() {
  ...
  for (var i = (store.currentPage - 1) * pagingSize; i < store.currentPage * pagingSize; i++) {
  ...
  var maxPage = Object.values(newsFeed).length / pagingSize;
  template = template.replace('{{__news_feed__}}', newsList.join(''));
  template = template.replace('{{__prev_page__}}', store.currentPage > 1 ? store.currentPage - 1 : 1);
  template = template.replace('{{__next_page__}}', store.currentPage + 1 <= maxPage ? store.currentPage + 1 : store.currentPage);
