...
export default class NewsFeedView extends View {
    ...
    render(): void {
        ...
        const maxPage = this.feeds.length / window.store.pagingSize;
    
        this.setTemplateData('news_feed', this.getHtml());
        this.setTemplateData('prev_page', String(window.store.currentPage > 1 ? window.store.currentPage -1 : 1));
        this.setTemplateData('next_page', String(window.store.currentPage + 1 <= maxPage ? window.store.currentPage + 1 : window.store.currentPage));
